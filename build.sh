...
CURRENT_VERSION=$(grep "Version:" wp-crowdfundtime.php | awk -F': ' '{print $2}' | tr -d '\r')

if [ -z "$CURRENT_VERSION" ]; then
  echo "Error: Could not find current version number in wp-crowdfundtime.php"
  exit 1
fi
...
# Update the version in the header line of wp-crowdfundtime.php using perl
perl -pi -e "s/^(\s*\*\s*Version:\s*)$CURRENT_VERSION/\${1}$NEW_VERSION/" wp-crowdfundtime.php
if [ $? -ne 0 ]; then
    echo "Error: Failed to update header version in wp-crowdfundtime.php"
    exit 1
fi

# Update the version in the define line of wp-crowdfundtime.php using perl
perl -pi -e "s/^(define\('WP_CROWDFUNDTIME_VERSION',\s*')$CURRENT_VERSION('\);)/\${1}$NEW_VERSION\${2}/" wp-crowdfundtime.php
if [ $? -ne 0 ]; then
    echo "Error: Failed to update define version in wp-crowdfundtime.php"
    exit 1
fi
